...
if __name__ == '__main__':
    ...
    l2_weight = 1e-3
    learning_rate = 0.02
    logPath = os.path.join(log_dir, "svd_k10_7")
    ...
    p = tf.Variable(1e-2 * tf.random.uniform([movie_count, k], 0, 1, dtype=tf.float32), name="user_matrix")
    q = tf.Variable(1e-2 * tf.random.uniform([tag_count, k], 0, 1, dtype=tf.float32), name="item_matrix")
    ...
    l2_all = l2_target_p + l2_target_q + l2_weight*l2_target_bu + l2_weight*l2_target_bi
