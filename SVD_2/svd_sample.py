...
if __name__ == '__main__':
    ...
    para_file_path = os.path.join(os.getcwd(), 'settings', '0102_2.conf')
    ...
    b_user = tf.Variable(tf.random_uniform([movie_count, 1], -1e-1, 1e-1, dtype=tf.float32), name="b_user")
    b_item = tf.Variable(tf.random_uniform([tag_count, 1], -1e-1, 1e-1, dtype=tf.float32), name="b_item")
    ...
    learning_rate = tf.train.exponential_decay(learning_rate, current_step, 40000, 0.96, staircase=True)
