...
if __name__ == '__main__':
    ...
    # rate_matrix = np.load(fillDir(data_dir, 'another_review_matrix.npy', missing_n))
    ...
    count = 0
    special_map = dict()
    movie_list = [1] * movie_count
    movie_list_log = [1] * movie_count
    ...
    neg_index_set = set()
    for i in range(sample_n*len(rate_list)):     
        while True:   
            movie_index = np.random.randint(movie_count)
            # movie_index = np.random.choice(movie_count, size=1, replace=False, p=movie_list_log)[0]
            tag_index = np.random.randint(tag_count)

            if rate_matrix[movie_index][tag_index] == 0 and another_review_matrix[movie_index][tag_index] != 1 and (movie_index,tag_index) not in neg_index_set:
                neg_index_set.add((movie_index,tag_index))
                rate_index_list.append([movie_index,tag_index])
                rate_list.append(0)
                break
    ...
    with sess.as_default():
        ...
        for step in range(total_step):
            ...
            index = np.random.randint(len(rate_list),size=1)
        ...
        fileName = "fixed_log_{}".format(sample_n)
