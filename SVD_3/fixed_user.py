...
if __name__ == '__main__':
    ...
    para_file_path = os.path.join(os.getcwd(), 'settings', '0318_5.conf')
    ...
    for i in range(sample_n*len(rate_list)):     
        while True:
            index = np.random.randint(len(special_map))    
            movie_index = special_map[index]
            tag_index = np.random.randint(tag_count)
            if rate_matrix[movie_index][tag_index] == 0 and (movie_index,tag_index) not in neg_index_set:
                if another_review_matrix[movie_index][tag_index] != 1:
    ...
    with sess.as_default():
        ...
        fileName = "fixed_user_{}_{}".format(missing_n, sample_n)
