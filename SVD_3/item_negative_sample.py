...
if __name__ == '__main__':
    ...
    para_file_path = os.path.join(os.getcwd(), 'settings', '0327_5.conf')
    ...
    # another_review_matrix = np.load('another_review_matrix.npy')
    ...
    total = 0
    count = 0
    special_count = [0 for i in range(tag_count)]
    ...
    total_length = len(rate_list)
    special_total = 0
    for i in range(tag_count):
        special_count[i] = 1.0 / (special_count[i] + 1)
        special_total += special_count[i]
    for i in range(tag_count):
        special_count[i] /= special_total
    ...
    b_user = tf.Variable(np.loadtxt(os.path.join(log_dir, 'svd_0102', 'b_user')).reshape(-1, 1).astype(np.float32),
                         name="b_user")
    b_item = tf.Variable(np.loadtxt(os.path.join(log_dir, 'svd_0102', 'b_item')).reshape(-1, 1).astype(np.float32),
                         name="b_item")
    community_cluster_y = tf.Variable(tf.random_uniform([cluster_k, k], -1e-4, 1e-4, dtype=tf.float32),
                                      name="community_cluster_y")
    ...
    with sess.as_default():
        ...
        total_step = int(5 * (sample_n + 1) * len(rate_list))
        print("total step : {}".format(total_step))
        for step in range(total_step):
            selection = np.random.randint(sample_n + 1)
            if selection == 0:
                # print "positive"
                index = np.random.randint(len(rate_list), size=batch_size)
                ...
                for i in range(1):
                    while True:
                        tag_index = np.random.choice(tag_count, 1, special_count)[0]
            ...
            # print len(neg_index)
            # neg_index = getSample(neg_rate_list, n=8)
        ...
        fileName = "item_normal_unfixed_sample_{}_{}".format(missing_n, sample_n)
