...
if __name__ == '__main__':
    ...
    para_file_path = os.path.join(os.getcwd(), 'settings', '1229_3.conf')
    ...
    # b_user = tf.Variable(1e-3 * tf.random_uniform([movie_count, 1], -1, 1, dtype=tf.float32), name="b_user")
    # b_item = tf.Variable(1e-3 * tf.random_uniform([tag_count, 1], -1, 1, dtype=tf.float32), name="b_item")
    ...
    with sess.as_default():
        ...
        saveMatrix(k, result_matrix, "result_matrix_2.npy")
        ...
        np.save(os.path.join(logPath, "p.npy"), value_p)
        np.save(os.path.join(logPath, "q.npy"), value_q)
