...
def saveMatrix(k, matrix, fileName = None):
    ...
    filePath = os.path.join(output_dir, 'svd_plus', fileName)
...
if __name__ == '__main__':
    ...
    para_file_path = os.path.join(os.getcwd(), 'settings', '0122.conf')
    ...
    logPath = fillDir(new_log_dir, res['logpath'], missing_n)
    ...
    rate_matrix = np.load(fillDir(data_dir, 'ori_matrix_sample_{}.npy'.format(missing_n), missing_n))
    ...
    b_item = tf.Variable(np.loadtxt(os.path.join(log_dir, 'svd_0102', 'b_item')).reshape(-1, 1).astype(np.float32),
                         name="b_item")
    tag_y = tf.Variable(tf.random_uniform([tag_count, k], -1e-4, 1e-4, dtype=tf.float32), name="tag_y")
    ...
    with tf.name_scope('pre') as scope:
        ...
        theta_N = tf.reduce_sum(theta_N_rows, axis=1, keepdims=True)                       # theta-N(i)  （？，1）
    ...
    with tf.name_scope('loss') as scope:
        ...
        l2_all = l2_weight*l2_target_p + 10 * l2_weight*l2_target_q + l2_b_extra * l2_weight*l2_target_bu + \
                 10 * l2_b_extra * l2_weight*l2_target_bi + l2_extra * l2_weight*l2_target_theta_y_sum
    ...
    with sess.as_default():
        ...
        for cur_epoch in range(int(epoch)):
            ...
            for step in range(math.floor(total_length / batch_size)):
                ...
                movie_index_choice = np.random.randint(movie_count)
                neg_index = []
                for j in range(neg_num):
                    while True:
                        movie_index = movie_index_choice[j]
                        tag_index = np.random.randint(tag_count)
                        if rate_matrix[movie_index][tag_index] != 1:
        ...
        fileName = "unfixed_user_minibatch{}_{}".format(missing_n, sample_n)
