...
if __name__ == '__main__':
    ...
    para_file_path = os.path.join(codedir, 'settings', '0329_wtbatch', '1.conf')
    ...
    with sess.as_default():
        ...
        for step in range(int(1)):
            ...
            sess.run(train, feed_dict={x: (x_data[permutation]).reshape(-1, 2), y: (y_data[permutation]).reshape(-1, 1)})

            neg_index = getSample(neg_rate_list, n=4*len(rate_list))
