...
if __name__ == '__main__':
    ...
    para_file_path = os.path.join(os.getcwd(), 'settings', '0430_minibatch_100.conf')
    ...
    movie_list = np.sum(np.abs(rate_matrix), 1)
    ...
    community_cluster_y = tf.Variable(tf.random_uniform([cluster_k, k],  -0.1, 0.1, dtype=tf.float32), name="community_cluster_y")
    ...
    with tf.name_scope('loss') as scope:
        ...
        l2_all = l2_weight*l2_target_p + l2_weight*l2_target_q + l2_b_extra * l2_weight*l2_target_bu + l2_b_extra * l2_weight*l2_target_bi + l2_extra * l2_weight*l2_target_theta_y_sum    # c
    ...
    learning_rate = tf.train.exponential_decay(learning_rate, current_step, 10000, 0.99, staircase=True)
    ...
    with sess.as_default():
        ...
        fileName = "unfixed_user_minibatch{}_{}_componentc".format(missing_n, sample_n)
